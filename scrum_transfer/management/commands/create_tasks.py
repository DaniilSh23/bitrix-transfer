...
from scrum_transfer.models import Settings, Scrums, Backlog, BitrixUsers, Sprint, ScrumTask
                ...
                for j_cloud_task in i_cloud_tasks.get('result').get('tasks'):
                    ...
                    if closed_by:
                        try:
                            ...
                            logger.warning(f'\t\tНе найден коробочный ID юзера для создания задачи в коробке.'
                                           f'Параметр для создания задачи == CLOSED_BY | '
                                           f'Облачный ID юзера == {j_cloud_task.get("closedBy")}'
                                           f'\nТекст ошибки: {error}'
                                           f'ЗАДАЧА СОЗДАНА НЕ БУДЕТ!')
                            continue
                    ...
                    if box_entity_id:
                        ...
                        params = {
                            ...
                            'fields': {
                                ...
                                'epicId': None if str(j_cloud_scrum_task.get('result').get('epicId')) == '0'
                                else str(j_cloud_scrum_task.get('result').get('epicId')),
                    ...
                    logger.info(f'\t\tОжидание 2 сек. перед итерацией по след. задаче.')
